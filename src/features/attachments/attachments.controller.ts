...
export async function createForPayment(req: Request, res: Response): Promise<void> {
  const attachments = await createPaymentAttachments(req.params.paymentId as string, uploadedFiles(req));
  res.status(201).json({ data: attachments });
}
...
export async function remove(req: Request, res: Response): Promise<void> {
  await deleteAttachment(req.params.attachmentId as string);
  res.status(204).send();
}
